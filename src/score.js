...
/* §11. El orden de los chequeos importa: el verdadero primero, y el "cualquier
   otro caso" cae al 1 o al 3 segun C >= 2. */
export function evaluarFinal(escapo, gemas, comps){
  if (!escapo) return 0;
  const G = gemas, C = comps;
  if (C === 4 && G >= CONFIG.gemasUmbralVerdadero) return 4;
  if (C <= 1 && G <  CONFIG.gemasUmbral) return 1;
  if (C <= 1 && G >= CONFIG.gemasUmbral) return 2;
  if (C >= 3 && G <  CONFIG.gemasUmbral) return 3;
  return C >= 2 ? 3 : 1;
}
...
export function pistaDeFinal(datos, vistos){
  ...
  if (falta(3) && C < 2)  return { clave: 'pista.3', params: { c: C } };
